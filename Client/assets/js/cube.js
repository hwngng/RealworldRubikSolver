(function () {
	...
	class Cube {
	    ...
	    for (x = 0; x < this.size; x++) {
	      for (y = 0; y < this.size; y++) {
	        for (z = 0; z < this.size; z++) {
	          // update
	          this.pos2PieceId[[x, y, z]] = idPiece++;
	  ...
	  loadFromData(data) {
	  	if (data.size && data.names && data.positions && data.rotations) {
	  		this.size = data.size;
	...
	class Controls {
	  constructor(game) {
	    ...
	    this.flipEasings = [
	      Easing.Power.Out(3),
	      Easing.Sine.Out(),
	      Easing.Power.Out(1.5)//Easing.Back.Out(1.5)
	    ];
	    this.flipSpeeds = [200, 600, 800];
	  ...
	  rotateLayer(rotation, scramble, callback) {
		...
		const duration = this.flipSpeeds[config] * ((Math.abs(rotation) > (Math.PI / 2 + 0.1)) ? 1.7 : 1);
	    ...
	    if (type === "LAYER") {
	      ...
	      this.rotateLayer(move.angle, false, layer => {
			...
			if (this.workList.length != 0) {
				let move = this.workList.shift();
				this.keyboardMove("LAYER", move, () => {});
			}
	      });
	    } else if (type === "CUBE") {
	      this.flipAxis = new THREE.Vector3();
	  ...
	  randomCube() {
	    ...
	    // var converted = this.game.scrambler.convertMove(x);
	...
	class Scrambler {
	  ...
	  convertMove(move) {
	    ...
	    const angle = (Math.PI / 2) * -row * (modifier == "'" ? -1 : parseInt(modifier));
	...
	class Transition {
	  ...
	  init() {
	    ...
	    this.game.cube.animator.rotation.x = -Math.PI / 5;
	...
	class Timer extends Animation {
	  ...
	  update() {
	    ...
	    if (this.converted != old) {
	      ...
	      localStorage.setItem("theCube_bestMove", this.moveCount);
	      this.setText();
	    }
	  }
	  ...
	  setText() {
	    ...
	    this.game.dom.texts.timer.innerHTML = this.moveCount + " Moves";
	  }
	}
